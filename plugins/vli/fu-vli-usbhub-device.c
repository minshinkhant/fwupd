...
static gboolean
fu_vli_usbhub_device_realtek_setup (FuVliUsbhubDevice *self, GError **error)
{
	g_autoptr(FuDevice) dev = NULL;
	g_autoptr(GError) error_local = NULL;

	/* add child */
	dev = fu_vli_usbhub_realtek_device_new (self);
	if (!fu_device_probe (dev, error))
		return FALSE;
	if (!fu_device_setup (dev, &error_local)) {
		if (g_error_matches (error_local,
...
fu_vli_usbhub_device_setup (FuVliDevice *device, GError **error)
{
	...
	if (fu_device_has_custom_flag (FU_DEVICE (self), "has-realtek")) {
		if (!fu_vli_usbhub_device_realtek_setup (self, error))
			return FALSE;
	}
